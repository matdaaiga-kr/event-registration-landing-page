# 이벤트 등록 랜딩페이지 생성 템플릿 (AI-first)

## 목적
이 문서는 이벤트 폴더의 `index.md`, `images/`, (선택) `speakers.csv`를 입력으로 받아, Global Azure Bootcamp 예시와 동일한 레이아웃/스타일의 정적 등록 랜딩페이지 `index.html`을 생성하기 위한 표준 가이드입니다.
...
### 헤더(필수)
`time,profile,name,organisation,session-title,session-description`
...
- 히어로(배너) 이미지는 `images/` 안에서 다음 우선순위로 선택한다.
  1) 파일명에 `hero`가 포함된 이미지
  2) 파일명에 `landscape`가 포함된 이미지
...
## 디자인/스타일 규칙(Global Azure Bootcamp 스타일 유지)
